...
import { Award, CheckCircle2, AlertTriangle, ScanLine } from 'lucide-react';
...
export default function SubscriptionPage() {
  ...
  const { subscriptionStatus, subscriptionEndDate } = client;
        ...
        <Card className="shadow-lg">
          ...
          <CardContent className="space-y-4">
            <div className="flex items-center justify-between p-4 border rounded-lg bg-muted/50">
              ...
              <Badge variant={isSubscriptionActive ? 'default' : 'destructive'} className={cn(
                  'text-sm',
                  isSubscriptionActive ? 'bg-green-500/20 text-green-700 border-green-400' : 'bg-red-500/20 text-red-700 border-red-400'
              )}>
                {isSubscriptionActive ? <CheckCircle2 className="h-4 w-4 mr-1"/> : <AlertTriangle className="h-4 w-4 mr-1"/>}
